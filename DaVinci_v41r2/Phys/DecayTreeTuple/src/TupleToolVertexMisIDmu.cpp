...
// local
#include "TupleToolVertexMisIDmu.h"
...
//-----------------------------------------------------------------------------
// Implementation file for class : TupleToolVertexMisIDmu
//
...
DECLARE_TOOL_FACTORY( TupleToolVertexMisIDmu )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
  TupleToolVertexMisIDmu::TupleToolVertexMisIDmu( const std::string& type,
...
StatusCode TupleToolVertexMisIDmu::initialize()
{
...
StatusCode TupleToolVertexMisIDmu::fill( const Particle* mother
                                    , const Particle* P
                                    ...
                                    , Tuples::Tuple& tuple )
{
  ...
  Assert( P && mother && m_dist
          , "This should not happen, you are inside TupleToolVertexMisIDmu.cpp :(" );
  ...
  */
  ...
  do {
	...
	//	} 
        ...
        //
                ...
                pro_part = (*itmp);
                cout<<"X_ID part"<<endl;
                cout<<"ID of particle 2:"<< (*itmp)->particleID().pid()<<endl;
                }
                if (((*itmp)->particleID().abspid() == 13)  && count==4)
                {
                parts2Vertex.push_back(*itmp);
                mu_part = (*itmp);
                cout<<"Mu part"<<endl;
                cout<<"ID of particle 4:"<< (*itmp)->particleID().pid()<<endl;      
                }
                cout<<"Size of a vector"<<parts2Vertex.size()<<endl;
  ...
  StatusCode sc = m_pVertexFit->fit(parts2Vertex,Xmu_vert,*newP);
    ...
    //cout<<"Filling"<<endl;
    fillVertex(&Xmu_vert, prefix +"_Xmu_SV", tuple);
    fillVertex(PV, prefix + "_Xmu_PV", tuple);
    if(m_momcov)
    {
    const Gaudi::SymMatrix4x4 & mom_cov = newP->momCovMatrix ();
    const Gaudi::Matrix4x3 & posmom_cov = newP->posMomCovMatrix ();
    test &= tuple->matrix( prefix +  "_Xmu_MOM_COV_", mom_cov );
    test &= tuple->matrix( prefix +  "_Xmu_POSMOM_COV_", posmom_cov );
    }
    ...
    double LV_invmass;
    LV_invmass = (TLV_mu+TLV_pro).M2();
    test &= tuple->column(  prefix + "_Xmu_INVMASS", LV_invmass);
  ...
  //  test &= tuple->column(  prefix + "_Xmu_MCORR", mcorr);
  //  if(mcorr_errors.size() ==  2)
  //  {
  //  mcorrerr = mcorr_errors[0];
  //  mcorrfullerr = mcorr_errors[1];
  //  test &= tuple->column(  prefix + "_Xmu_MCORRERR", mcorrerr);
  //  test &= tuple->column(  prefix + "_Xmu_MCORRFULLERR", mcorrfullerr);
  //  }
  //  else
 //   {
 //   test &= tuple->column(  prefix + "_Xmu_MCORRERR", -1000.);
 //   test &= tuple->column(  prefix + "_Xmu_MCORRFULLERR", -1000.);
 //   }
 ...
 //   test &= tuple->column(  prefix + "_Xmu_Q2SOL1", q2_one);
 //   test &= tuple->column(  prefix + "_Xmu_Q2SOL2", q2_two);
 //   }
 //   else
 //   {
 //   test &= tuple->column(  prefix + "_Xmu_Q2SOL1", -1000.);
 //   test &= tuple->column(  prefix + "_Xmu_Q2SOL2", -1000.);
 //   }
...
//
//=========================================================================
StatusCode TupleToolVertexMisIDmu::fillVertex( const LHCb::VertexBase* vtx,
...
double TupleToolVertexMisIDmu::Mcorr(TLorentzVector Y, TVector3 M_dirn)
{
     double P_T = (Y.Vect()).Perp(M_dirn);
     return  sqrt(Y.M2() + P_T * P_T) + P_T;
}

std::vector<TLorentzVector> TupleToolVertexMisIDmu::recoNu(TLorentzVector Y, TVector3 M_dirn, double mass)
    {
	...
	// Calculate Xmu " "
	double E_Xmurest = sqrt(E_nurest * E_nurest + (Y).M2());
	double px_rest;
	...
	double B = - 2 * PXmu.Px()  * (E_nurest * E_Xmurest + px_rest * px_rest );
	double C =  - (E_nurest * E_Xmurest + px_rest * px_rest )*
	    (E_nurest * E_Xmurest + px_rest * px_rest ) +  (Y).E() * (Y).E() * PXmu.Py() * PXmu.Py();
...
std::vector<double> TupleToolVertexMisIDmu::Mcorr_errors(TVector3 v1, TVector3 v2, TLorentzVector p,  Gaudi::SymMatrix7x7 cov1, Gaudi::SymMatrix3x3 cov2 )
{
