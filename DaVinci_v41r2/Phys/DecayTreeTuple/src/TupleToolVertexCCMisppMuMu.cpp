...
// local
#include "TupleToolVertexCCMisppMuMu.h"
...
//-----------------------------------------------------------------------------
// Implementation file for class : TupleToolVertexCCMisppMuMu
//
...
DECLARE_TOOL_FACTORY( TupleToolVertexCCMisppMuMu )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
  TupleToolVertexCCMisppMuMu::TupleToolVertexCCMisppMuMu( const std::string& type,
...
StatusCode TupleToolVertexCCMisppMuMu::initialize()
{
...
StatusCode TupleToolVertexCCMisppMuMu::fill( const Particle* mother
                                    , const Particle* P
                                    ...
                                    , Tuples::Tuple& tuple )
{
  ...
  Assert( P && mother && m_dist
          , "This should not happen, you are inside TupleToolVertexCCMisppMuMu.cpp :(" );
      ...
      if(mumid==443)
     {
      ...
      if(!((*isource)->daughters().empty())){
        ...
        for( LHCb::Particle::ConstVector::const_iterator itmp = tmp.begin(); 
          ...
          target.push_back(*itmp);
//          cout<<"daugther Particle id: "<<(*itmp)->particleID().pid()<<" Wooooohoooooooooooooooooooooooo!"<<endl;
           if (((*itmp)->particleID().abspid()  == m_Xid) && ((*itmp)->charge() == mothercharge))
              {
              parts2Vertex.push_back(*itmp);
              mu_part = (*itmp);
...
// StatusCode sc2 = m_pVertexFit->fit(parts2Vertex,Xmu_vert,*newP);
    ...
    fillVertex(&Xmu_vert, prefix +"_ppMuMu_SV", tuple);
    fillVertex(PV, prefix + "_ppMuMu_PV", tuple);
    if(m_momcov)
    {
    const Gaudi::SymMatrix4x4 & mom_cov = newP->momCovMatrix ();
    const Gaudi::Matrix4x3 & posmom_cov = newP->posMomCovMatrix ();
    test &= tuple->matrix( prefix +  "_ppMuMu_MOM_COV_", mom_cov );
    test &= tuple->matrix( prefix +  "_ppMuMu_POSMOM_COV_", posmom_cov );
    }
    ...
    test &= tuple->column(  prefix + "_ppMuMu_INVMASS", LV_invmass);
    test &= tuple->column(  prefix + "_ppMuMu_INVMASSSQ", LV_invmasssq);
...
//    nu_slns = recoNu(TLV_newP, Mdirn, 5279.15); 
  ...
  //  test &= tuple->column(  prefix + "_ppMuMu_MCORR", mcorr);
  //  if(mcorr_errors.size() ==  2)
  //  {
  //  mcorrerr = mcorr_errors[0];
  //  mcorrfullerr = mcorr_errors[1];
  //  test &= tuple->column(  prefix + "_ppMuMu_MCORRERR", mcorrerr);
  //  test &= tuple->column(  prefix + "_ppMuMu_MCORRFULLERR", mcorrfullerr);
  //  }
  //  else
 //   {
 //   test &= tuple->column(  prefix + "_ppMuMu_MCORRERR", -1000.);
 //   test &= tuple->column(  prefix + "_ppMuMu_MCORRFULLERR", -1000.);
 //   }
 ...
 //   test &= tuple->column(  prefix + "_ppMuMu_Q2SOL1", q2_one);
 //   test &= tuple->column(  prefix + "_ppMuMu_Q2SOL2", q2_two);
 //   }
 //   else
 //   {
 //   test &= tuple->column(  prefix + "_ppMuMu_Q2SOL1", -1000.);
 //   test &= tuple->column(  prefix + "_ppMuMu_Q2SOL2", -1000.);
 //   }
...
//
//=========================================================================
StatusCode TupleToolVertexCCMisppMuMu::fillVertex( const LHCb::VertexBase* vtx,
...
double TupleToolVertexCCMisppMuMu::Mcorr(TLorentzVector Y, TVector3 M_dirn)
{
     double P_T = (Y.Vect()).Perp(M_dirn);
     return  sqrt(Y.M2() + P_T * P_T) + P_T;
}

std::vector<TLorentzVector> TupleToolVertexCCMisppMuMu::recoNu(TLorentzVector Y, TVector3 M_dirn, double mass)
    {
	...
	// Calculate Xmu " "
	double E_ppMuMurest = sqrt(E_nurest * E_nurest + (Y).M2());
	double px_rest;
	...
	double B = - 2 * PXmu.Px()  * (E_nurest * E_ppMuMurest + px_rest * px_rest );
	double C =  - (E_nurest * E_ppMuMurest + px_rest * px_rest )*
	    (E_nurest * E_ppMuMurest + px_rest * px_rest ) +  (Y).E() * (Y).E() * PXmu.Py() * PXmu.Py();
...
std::vector<double> TupleToolVertexCCMisppMuMu::Mcorr_errors(TVector3 v1, TVector3 v2, TLorentzVector p,  Gaudi::SymMatrix7x7 cov1, Gaudi::SymMatrix3x3 cov2 )
{
