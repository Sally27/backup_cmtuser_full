...
// local
#include "TupleToolVertexMisOSpmMuMu.h"
...
//-----------------------------------------------------------------------------
// Implementation file for class : TupleToolVertexMisOSpmMuMu
//
...
DECLARE_TOOL_FACTORY( TupleToolVertexMisOSpmMuMu )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
  TupleToolVertexMisOSpmMuMu::TupleToolVertexMisOSpmMuMu( const std::string& type,
...
StatusCode TupleToolVertexMisOSpmMuMu::initialize()
{
...
StatusCode TupleToolVertexMisOSpmMuMu::fill( const Particle* mother
                                    , const Particle* P
                                    ...
                                    , Tuples::Tuple& tuple )
{
  ...
  Assert( P && mother && m_dist
          , "This should not happen, you are inside TupleToolVertexMisOSpmMuMu.cpp :(" );
	  ...
	  for(LHCb::Particle::ConstVector::const_iterator isource = source.begin(); 
		 ...
		 // mothercharge = (*isource)->charge();
		  ...
		  if(mumid==521)
		  {
			  //     cout<<"I am here"<<endl;
                          mothercharge = (*isource)->charge();
                          cout<<"mother charge: "<< mothercharge;
			  ...
			  if(!((*isource)->daughters().empty())){
				  ...
				  for( LHCb::Particle::ConstVector::const_iterator itmp = tmp.begin(); 
					  ...
					  target.push_back(*itmp);
                                           ...
                                           cout<<"daugthers: " << (*itmp)->particleID().pid()<<endl;
	  ...
	  if(mumid==443)
	  {
		  ...
		  if(!((*isource)->daughters().empty())){
			  ...
			  for( LHCb::Particle::ConstVector::const_iterator itmp = tmp.begin(); 
				  ...
				  target.push_back(*itmp);
				  //          cout<<"daugther Particle id: "<<(*itmp)->particleID().pid()<<" Wooooohoooooooooooooooooooooooo!"<<endl;
				  if (((*itmp)->particleID().abspid()  == 13) && muon==0)
				  {
					  parts2Vertex.push_back(*itmp);
					  mu_part = (*itmp);
                                          muon=muon+1;
...
//
    fillVertex(&Xmu_vert, prefix +"_pmMuMu_SV", tuple);
    fillVertex(PV, prefix + "_pmMuMu_PV", tuple);
    if(m_momcov)
    {
    const Gaudi::SymMatrix4x4 & mom_cov = newP->momCovMatrix ();
    const Gaudi::Matrix4x3 & posmom_cov = newP->posMomCovMatrix ();
    test &= tuple->matrix( prefix +  "_pmMuMu_MOM_COV_", mom_cov );
    test &= tuple->matrix( prefix +  "_pmMuMu_POSMOM_COV_", posmom_cov );
    }
    ...
    test &= tuple->column(  prefix + "_pmMuMu_INVMASS", LV_invmass);
    test &= tuple->column(  prefix + "_pmMuMu_INVMASSSQ", LV_invmasssq);
...
//    nu_slns = recoNu(TLV_newP, Mdirn, 5279.15); 
  ...
  //  test &= tuple->column(  prefix + "_pmMuMu_MCORR", mcorr);
  //  if(mcorr_errors.size() ==  2)
  //  {
  //  mcorrerr = mcorr_errors[0];
  //  mcorrfullerr = mcorr_errors[1];
  //  test &= tuple->column(  prefix + "_pmMuMu_MCORRERR", mcorrerr);
  //  test &= tuple->column(  prefix + "_pmMuMu_MCORRFULLERR", mcorrfullerr);
  //  }
  //  else
 //   {
 //   test &= tuple->column(  prefix + "_pmMuMu_MCORRERR", -1000.);
 //   test &= tuple->column(  prefix + "_pmMuMu_MCORRFULLERR", -1000.);
 //   }
 ...
 //   test &= tuple->column(  prefix + "_pmMuMu_Q2SOL1", q2_one);
 //   test &= tuple->column(  prefix + "_pmMuMu_Q2SOL2", q2_two);
 //   }
 //   else
 //   {
 //   test &= tuple->column(  prefix + "_pmMuMu_Q2SOL1", -1000.);
 //   test &= tuple->column(  prefix + "_pmMuMu_Q2SOL2", -1000.);
 //   }
...
//
//=========================================================================
StatusCode TupleToolVertexMisOSpmMuMu::fillVertex( const LHCb::VertexBase* vtx,
...
double TupleToolVertexMisOSpmMuMu::Mcorr(TLorentzVector Y, TVector3 M_dirn)
{
     double P_T = (Y.Vect()).Perp(M_dirn);
     return  sqrt(Y.M2() + P_T * P_T) + P_T;
}

std::vector<TLorentzVector> TupleToolVertexMisOSpmMuMu::recoNu(TLorentzVector Y, TVector3 M_dirn, double mass)
    {
	...
	// Calculate Xmu " "
	double E_pmMuMurest = sqrt(E_nurest * E_nurest + (Y).M2());
	double px_rest;
	...
	double B = - 2 * PXmu.Px()  * (E_nurest * E_pmMuMurest + px_rest * px_rest );
	double C =  - (E_nurest * E_pmMuMurest + px_rest * px_rest )*
	    (E_nurest * E_pmMuMurest + px_rest * px_rest ) +  (Y).E() * (Y).E() * PXmu.Py() * PXmu.Py();
...
std::vector<double> TupleToolVertexMisOSpmMuMu::Mcorr_errors(TVector3 v1, TVector3 v2, TLorentzVector p,  Gaudi::SymMatrix7x7 cov1, Gaudi::SymMatrix3x3 cov2 )
{
