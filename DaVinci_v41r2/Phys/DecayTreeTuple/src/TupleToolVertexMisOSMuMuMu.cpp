...
// local
#include "TupleToolVertexMisOSMuMuMu.h"
...
//-----------------------------------------------------------------------------
// Implementation file for class : TupleToolVertexMisOSMuMuMu
//
...
DECLARE_TOOL_FACTORY( TupleToolVertexMisOSMuMuMu )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
  TupleToolVertexMisOSMuMuMu::TupleToolVertexMisOSMuMuMu( const std::string& type,
...
StatusCode TupleToolVertexMisOSMuMuMu::initialize()
{
...
StatusCode TupleToolVertexMisOSMuMuMu::fill( const Particle* mother
                                    , const Particle* P
                                    ...
                                    , Tuples::Tuple& tuple )
{
  ...
  Assert( P && mother && m_dist
          , "This should not happen, you are inside TupleToolVertexMisOSMuMuMu.cpp :(" );
...
//       cout<<"source id: "<<(*isource)->particleID().pid()<<" Wooooohoooooooooooooooooooooooo!"<<endl;
//       cout<<"source charge: "<<(*isource)->charge()<<" Wooooohoooooooooooooooooooooooo!"<<endl;
//       cout<<"MP"<<endl;

       mothercharge = (*isource)->charge();
       mumid = (*isource)->particleID().abspid();    
...
//          cout<<"daugther Particle id: "<<(*itmp)->particleID().pid()<<" Wooooohoooooooooooooooooooooooo!"<<endl;
           if (((*itmp)->particleID().abspid()  == m_Xid))
              {
              parts2Vertex.push_back(*itmp);
              pro_part = (*itmp);
              muon=(*itmp)->particleID().pid();
//              cout<<"X_ID part: "<<"ID of particle 1: "<< (*itmp)->particleID().pid()<<endl;
              }
      ...
      if(mumid==443)
     {
      ...
      if(!((*isource)->daughters().empty())){
        ...
        for( LHCb::Particle::ConstVector::const_iterator itmp = tmp.begin(); 
          ...
          target.push_back(*itmp);
//          cout<<"daugther Particle id: "<<(*itmp)->particleID().pid()<<" Wooooohoooooooooooooooooooooooo!"<<endl;
           if (((*itmp)->particleID().pid()  != muon) && flag==1)
              {
              parts2Vertex.push_back(*itmp);
              mu_part = (*itmp);
//              cout<<"Mu part: "<<"ID of particle 1: "<< (*itmp)->particleID().pid()<<endl;
              }
...
//              cout<<"Mu part: "<<"ID of particle 1: "<< (*itmp)->particleID().pid()<<endl;
  ...
  LHCb::Particle *newP = new LHCb::Particle(LHCb::ParticleID(m_motherid || -m_motherid));
...
// StatusCode sc2 = m_pVertexFit->fit(parts2Vertex,Xmu_vert,*newP);
    ...
    //cout<<"Filling"<<endl;
     cout<<"Filling Mu Mu Mu!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;
//    LHCb::Particle::ConstVector bla = (*parts2Vertex)->daughtersVector();
    for( LHCb::Particle::ConstVector::const_iterator hello = parts2Vertex.begin();
        hello!=parts2Vertex.end(); hello++){
        cout<<"vertex id: "<<(*hello)->particleID().pid()<<endl;
        }
    



    fillVertex(&Xmu_vert, prefix +"_MuMuMu_SV", tuple);
    fillVertex(PV, prefix + "_MuMuMu_PV", tuple);
    if(m_momcov)
    {
    const Gaudi::SymMatrix4x4 & mom_cov = newP->momCovMatrix ();
    const Gaudi::Matrix4x3 & posmom_cov = newP->posMomCovMatrix ();
    test &= tuple->matrix( prefix +  "_MuMuMu_MOM_COV_", mom_cov );
    test &= tuple->matrix( prefix +  "_MuMuMu_POSMOM_COV_", posmom_cov );
    }

    Gaudi::LorentzVector LV_newP,LV_mu, LV_XID, LV_last ;
    ...
    LV_last = last_part->momentum();
    TLorentzVector TLV_last(LV_last.px(),LV_last.py(),LV_last.pz(),LV_last.E());


    
    double LV_invmass, LV_invmasssq;
    LV_invmasssq = (TLV_mu+TLV_pro+TLV_last).M2();
    LV_invmass = (TLV_mu+TLV_pro+TLV_last).M();
    test &= tuple->column(  prefix + "_MuMuMu_INVMASS", LV_invmass);
    test &= tuple->column(  prefix + "_MuMuMu_INVMASSSQ", LV_invmasssq);
 ...
 //   nu_slns = recoNu(TLV_newP, Mdirn, 5279.15); 
    ...
    test &= tuple->column(  prefix + "_MuMuMu_MCORR", mcorr);
    if(mcorr_errors.size() ==  2)
    {
    mcorrerr = mcorr_errors[0];
    mcorrfullerr = mcorr_errors[1];
    test &= tuple->column(  prefix + "_MuMuMu_MCORRERR", mcorrerr);
    test &= tuple->column(  prefix + "_MuMuMu_MCORRFULLERR", mcorrfullerr);
    }
    else
    {
    test &= tuple->column(  prefix + "_MuMuMu_MCORRERR", -1000.);
    test &= tuple->column(  prefix + "_MuMuMu_MCORRFULLERR", -1000.);
    }
 ...
 //   test &= tuple->column(  prefix + "_MuMuMu_Q2SOL1", q2_one);
 //   test &= tuple->column(  prefix + "_MuMuMu_Q2SOL2", q2_two);
 //   }
 //   else
 //   {
 //   test &= tuple->column(  prefix + "_MuMuMu_Q2SOL1", -1000.);
 //   test &= tuple->column(  prefix + "_MuMuMu_Q2SOL2", -1000.);
 //   }
...
//
//=========================================================================
StatusCode TupleToolVertexMisOSMuMuMu::fillVertex( const LHCb::VertexBase* vtx,
                                          ...
                                          Tuples::Tuple& tuple ) const
{
  ...
  else
  {
    ...
    test &= tuple->column(  vtx_name + "_CHI2perNDOF", vtx->chi2PerDoF() );
    if(m_vcov)
    {
...
double TupleToolVertexMisOSMuMuMu::Mcorr(TLorentzVector Y, TVector3 M_dirn)
{
     double P_T = (Y.Vect()).Perp(M_dirn);
     return  sqrt(Y.M2() + P_T * P_T) + P_T;
}

std::vector<TLorentzVector> TupleToolVertexMisOSMuMuMu::recoNu(TLorentzVector Y, TVector3 M_dirn, double mass)
    {
	...
	// Calculate Xmu " "
	double E_MuMuMurest = sqrt(E_nurest * E_nurest + (Y).M2());
	double px_rest;
	...
	double B = - 2 * PXmu.Px()  * (E_nurest * E_MuMuMurest + px_rest * px_rest );
	double C =  - (E_nurest * E_MuMuMurest + px_rest * px_rest )*
	    (E_nurest * E_MuMuMurest + px_rest * px_rest ) +  (Y).E() * (Y).E() * PXmu.Py() * PXmu.Py();
...
std::vector<double> TupleToolVertexMisOSMuMuMu::Mcorr_errors(TVector3 v1, TVector3 v2, TLorentzVector p,  Gaudi::SymMatrix7x7 cov1, Gaudi::SymMatrix3x3 cov2 )
{
