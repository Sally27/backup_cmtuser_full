...
#    relatedInfoToolsConfig4DD_Line = filter_relatedinfo_locations(config['RelatedInfoTools'],[selB2DD,selD,selPionsForD,selKaonsForD])
    self.DD_Line        = StrippingLine(name+"_DD_Line",
                                        ...
                                        RelatedInfoTools = [
                                          ...
                                          { "Type" : "RelInfoConeIsolation",  "Location"  : "BVars_ConeIsolation", "ConeSize" : 0.5,
                                                    "DaughterLocations" : {
                                                      "B0 -> ^D+ D-" : "DVars_ConeIsolation_0",
                                                      "B0 -> D+ ^D-" : "DVars_ConeIsolation_1"
                                                    }
                                          }       
                                        ],
 ...
 #   relatedInfoToolsConfig4TauMu_piSS_TOSLine = filter_relatedinfo_locations(config['RelatedInfoTools'],[selB2TauMupiSSTOS,selMuons,selTauSS,selPionsForD])
    self.TauMu_piSS_TOSLine     = StrippingLine(name+"_TauMu_piSS_TOSLine",
                                                ...
                                                RelatedInfoTools = [
                                                  ...
                                                  { "Type" : "RelInfoConeIsolation",  "Location"  : "BVars_ConeIsolation", "ConeSize" : 0.5,
                                                    "DaughterLocations" : {
                                                      "[B0 -> ^tau+ mu-]CC" : "TauVars_ConeIsolation_0",
                                                      "[B0 -> tau+ ^mu-]CC" : "MuVars_ConeIsolation"
                                                    }
                                                  }
                                                ],
...
#                                       Preambulo        = preambulo)    
     ...
     _CombineTau = DaVinci__N3BodyDecays(
                   ...
                   CombinationCut = "in_range ( %(MASS_LOW_TAU)s , AM , %(MASS_HIGH_TAU)s ) & ( APT          >  %(APT_D)s )"\
                                    " & ( AMAXDOCA('') <  %(AMAXDOCA_D)s )"\
                                    " & ( ANUM(PT      >  %(MaxPT_D)s)>=1 )" %config,
                   MotherCut =   "( PT                >  %(PT_D)s ) & ( BPVDIRA         >  %(DIRA_D)s )"\
                                 "& ( VFASPF(VCHI2)   <  %(VCHI2_D)s ) "\
                                 "& ( BPVVDCHI2       >  %(FDCHI2_D)s )"\
                                 "& ( in_range ( %(VDRHOmin_D)s, BPVVDRHO, %(VDRHOmax_D)s ) ) & ( BPVVDZ          >  %(VDZ_D)s )"\
                                 "& ( in_range ( %(MASS_LOW_TAUmother)s, M, %(MASS_HIGH_TAUmother)s ) )" %config 
                   ) 
