...
_B = CombineParticles('BFilter',
                        DecayDescriptor = "[B+ ->  pi+ pi- pi0 mu+]cc",
...
BSel = Selection(name = 'BFilterSel',
                        Algorithm = _B,
                        RequiredSelections = [ MuonFilterSel, PionFilterSel, _stdPi0])
...
tuple.addBranches ({
#      B+ -> ^(D0 -> ^(KS0 -> ^pi+ ^pi-) ^pi+ ^pi- ^pi0) ^mu+
      "mu3" : "[B+ -> (D~0 -> (KS0 -> pi+ pi-) pi+ pi- pi0) ^mu+]CC",
      "mu1" : "[B+ -> (D~0 -> (KS0 -> pi+ pi-) ^pi+ pi- pi0) mu+]CC ",
      "mu2" : "[ B+ -> (D~0 -> (KS0 -> pi+ pi-) pi+ ^pi- pi0) mu+]CC",
      "Bplus" : "^([B+ -> ^(D~0 -> ^(KS0 -> ^pi+ ^pi-) ^pi+ ^pi- ^pi0) ^mu+]CC)"
})
...
tuple.Decay = "[B+ -> ^(D0 -> ^(KS0 -> ^pi+ ^pi-) ^pi+ ^pi- ^pi0) ^mu+]CC"
...
from Configurables import DaVinci
DaVinci().TupleFile = "Bplus23munuNew.root"
...
IOHelper().inputFiles(['./00042232_00000061_2.AllStreams.dst'], clear=True)
