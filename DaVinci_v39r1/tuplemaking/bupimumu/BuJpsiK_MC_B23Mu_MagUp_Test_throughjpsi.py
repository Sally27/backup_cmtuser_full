...
tuple = DecayTreeTuple("B2JpsiPi_Tuple")
...
LoKi_Bplus.Preambulo += [
  "muE = CHILD(E, '[B+ -> (J/psi(1S) -> mu+ ^mu-)  pi+]CC' )",
  "muP = CHILD(P, '[B+ -> (J/psi(1S) -> mu+ ^mu-)  pi+]CC' )",
  "muE_aspi = sqrt(muP*muP + 139*139)",
  "muPX = CHILD(PX, '[B+ -> (J/psi(1S) -> mu+ ^mu-)  pi+]CC' )",
  "muPY = CHILD(PY, '[B+ -> (J/psi(1S) -> mu+ ^mu-)  pi+]CC' )",
  "muPZ = CHILD(PZ, '[B+ -> (J/psi(1S) -> mu+ ^mu-)  pi+]CC' )",
  "KP = CHILD(P, '[B+ -> J/psi(1S) ^pi+]CC' )",
  "KE_asmu = sqrt(KP*KP + 105*105)",
  "KE = sqrt(KP*KP + 497*497)",
  "KPX = CHILD(PX, '[B+ -> J/psi(1S) ^pi+]CC' )",
  "KPY = CHILD(PY, '[B+ -> J/psi(1S) ^pi+]CC' )",
  "KPZ = CHILD(PZ, '[B+ -> J/psi(1S) ^pi+]CC' )",
  "kmuE = muE + KE_asmu",
  "DE = muE_aspi + KE",
  "PX = muPX + KPX",
  "PY = muPY + KPY",
...
#       'ENDVERTEX_CHI2' : "VFASPF(VCHI2/VDOF)",
       'PiMu_Jpsi' : "sqrt(kmuE*kmuE-(PX*PX+PY*PY+PZ*PZ))",
       'PiMu_D' : "sqrt(DE*DE-(PX*PX+PY*PY+PZ*PZ))",
       #'DimuM' : "DTF_FUN(CHILD(M,'[B+ -> ^J/psi(1S) pi+]CC'),True,'B+')"
       'TAU' : "BPVLTIME()",
       'DIRA_OWNPV' : "BPVDIRA",
       'FD_CHI2' : "BPVVDCHI2",
       'ENDVERTEX_CHI2' : "VFASPF(VCHI2/VDOF)",
...
from Configurables import DaVinci
DaVinci().TupleFile = "BuPiMuMuMC_thorughjpsi.root"
...
IOHelper().inputFiles(['./00036658_00000014_1.allstreams.dst'], clear=True)
