...
class GenericOptionFile(object):
    ...
    def AddGenXiccProductionToolXibcOptions( self , eventtype ):
        self.AddOptionValue( 'Generation.{0}.{1}Production.Commands'.format( eventtype.Sample(), eventtype.Production() ) ,
                             '["mixevnt imix 1", "loggrade ivegasopen 0", "loggrade igrade 1", "vegasbin nvbin 300", "counter xmaxwgt 5000000", "confine pscutmin 0.0", "confine pscutmax 7.0"]')
...
class PythonOptionFile( GenericOptionFile ):
    ...
    def AddPGunInfo( self, pdgIDs, eventtype ):
        ...
        if eventtype.Cuts() != 'None':
           self.Write( [ 'pgun.GenCutTool = "{0}"'.format( eventtype.Cuts() ) ] )
        else:
           self.Write( [ 'pgun.GenCutTool = ""' ] )
        if eventtype.Cuts() == 'LHCbAcceptance' and eventtype.HasCutsOptions()== False:
            self.Write( [ '', 
...
# and extra option includes TracksInAccWithMinP.py 
        ...
        self.Write( [ '',
                      ...
                      'pgun.EventType = {0}'.format( eventtype.EventTypeNumber() ) ] )
...
class EventType:
    ...
    def DecodeDecayFile( self ):
        ...
        if self.KeywordDictionary['Cuts'][0] == "\'" or \
           self.KeywordDictionary['Cuts'][0] == "\"":
            self.KeywordDictionary['Cuts'] = self.KeywordDictionary['Cuts'][1:]
        if self.KeywordDictionary['Cuts'][len(self.KeywordDictionary['Cuts'])-1] == "\'" or \
           self.KeywordDictionary['Cuts'][len(self.KeywordDictionary['Cuts'])-1] == "\"":
            self.KeywordDictionary['Cuts'] = self.KeywordDictionary['Cuts'][0:len(self.KeywordDictionary['Cuts'])-1]
    ...
    def Sample( self ):
        ...
        if self.EventTypeNumber()[0:2] == '30':
            ...
            if int(self.EventTypeNumber()[1]) == 0:
                sample = "Inclusive"
                ...
                sample = "SignalPlain"
            elif self.EventTypeNumber()[0:2] == '39': ## phi production
                sample = "SignalPlain"
            elif int(self.EventTypeNumber()[0]) == 1 and int(self.EventTypeNumber()[1]) in (1, 2, 3, 6, 7):
                sample = "SignalRepeatedHadronization"
                if int(self.EventTypeNumber()[1]) == 6 and int(self.EventTypeNumber()[6]) == 6 : ## Xibc baryons
                    sample = "Special"
...
def genParticleGuns( eventtype ):
    ...
    if not (eventtype.S() == '9' and eventtype.D() == '9'):
...
def pgunPID( eventtype ):
    ...
    elif eventtype.D() == '9': value = value
    else:
...
#            eventtype.OptionFile.AddGenXiccBeamMomentum( eventtype )
            if int(eventtype.EventTypeNumber()[1]) == 6 and int(eventtype.EventTypeNumber()[6]) == 6 : ## Xibc baryons
