    ...
    while i < len(list_base):
        if isinstance(list_base[i], list):
            if len(list_base[i]) == 1:
                if ( list_base[i][0] == "os" or list_base[i][0] == "nos" ) and i > 0:
                    list_base[i-1] = list_base[i-1]+"["+list_base[i][0]+"]"
                    list_base.pop(i)
                    i-=1
                elif i > 0:
                    list_base[i-1] = list_base[i-1]+"("+list_base[i][0]+")"
...
def cleanList( stringA, list_base ):
    ...
    if(not list_base.count(tag1) == list_base.count(tag2)):
        warning("Mismatched parentheses in descriptor, aborting cleanup")
        return []
